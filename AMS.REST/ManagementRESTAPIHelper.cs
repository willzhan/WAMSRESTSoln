...
namespace AMS.REST
{
    public class ManagementRESTAPIHelper
    {
        ...
        public AccountDetails GetAccountDetails(string mediaServiceName)
        {
            var clientCert = GetClientCertificate();

            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(string.Format("{0}/{1}/services/mediaservices/Accounts/{2}",
                Endpoint, SubscriptionId, mediaServiceName));
            request.Method = "GET";
            request.Headers.Add("x-ms-version", "2011-10-01");
            request.Accept = "application/json";
            request.ContentType = "application/json";
            ...
            Console.WriteLine(string.Format("Try to get the details of accountName[{0}].", mediaServiceName));
            using (var response = (HttpWebResponse)request.GetResponse())
            {
                var stream1 = response.GetResponseStream();
                DataContractJsonSerializer ser = new DataContractJsonSerializer(typeof(AccountDetails));
                accountDetails = (AccountDetails)ser.ReadObject(stream1);
                Console.WriteLine("Deserialized back:");
                Console.WriteLine("Media Service Name = {0}", accountDetails.AccountName);
                Console.WriteLine("Media Service Key = {0}", accountDetails.AccountKeys.Primary);
                Console.WriteLine("Media Service Storage Name = {0}", accountDetails.StorageAccountName);
            }
